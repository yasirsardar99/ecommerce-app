            ...
            <div className='d-flex align-items-center'>
...

                {
                    isAuthenticated
                        ? (
                            <>                       
                            <img className='userprofile mx-2' src={user.picture} alt="user-profile" />
                             <button className="btn btn-sm btn-dark  btn-outine-warning mx-2 my-2 my-sm-0 " onClick={() => logout({ logoutParams: { returnTo: window.location.origin } })}>Log Out</button>
                            </>
                          ) 
                        :
                        <button className="btn btn-sm btn-warning btn-outine-light my-2 my-sm-0 " onClick={() => loginWithRedirect()}>Log In</button>
                }
